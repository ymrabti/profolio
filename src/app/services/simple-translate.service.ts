...
const TRANSLATIONS: { [key: string]: TranslationData } = {
  ...
  fr: {
    hero: {
      ...
      description:
        'Développeur Full Stack et Spécialiste SIG avec une expertise en technologies web modernes, développement mobile et solutions géospatiales. Passionné par la création de plateformes innovantes et la contribution à la communauté des développeurs.',
    ...
    skills: {
      ...
      subtitle:
        "Technologies et outils que j'utilise pour créer des solutions extraordinaires",
      categories: {
        dev: 'Développement',
        ops: 'DevOps',
...
export class SimpleTranslateService {
  ...
  get(key: string): Observable<string | any> {
    const translation = this.getNestedProperty(
  ...
  instant(key: string): string | any {
    return (
      this.getNestedProperty(TRANSLATIONS[this.currentLanguage], key) || key
    );
  }
